...
async def get_departments_by_college(college_id: int) -> List[Department]:
    """Retrieve all departments for a college."""
    try:
        response = supabase.table("departments").select("*").eq("college_id", college_id).execute()
...
async def update_department(department_id: int, department: DepartmentUpdate) -> Department:
    """Update a department's details."""
    try:
        response = supabase.table("departments").update(department.dict(exclude_unset=True)).eq("id", department_id).execute()
        if not response.data:
