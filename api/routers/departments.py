...
async def get_department(department_id: int, _=Depends(get_current_admin)):
...
async def list_departments(college_id: int = None):
...
async def update_department_details(department_id: int, department: DepartmentUpdate, _=Depends(get_current_admin)):
    """Update a department's details."""
    return await update_department(department_id, department)

@router.delete("/{department_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_department_record(department_id: int, _=Depends(get_current_admin)):
